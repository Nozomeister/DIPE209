...
from paths import root_dir, mkdir_if_not_exist, submission_dir #changing input and output from 1 folder
import pandas as pd
...
task12_validation_img = 'ISIC-2017_Test_v2_Data'#change from ISIC2018_Task1-2_Validation_Input
task12_test_img = 'Input'
...
task12_test_img_dir = os.path.join(submission_dir, task12_test_img) #changing to submission
...
task12_test_images_npy_prefix = 'task12_test_images_2017'
...
def load_task12_test_images(output_size=None):
    ...
    #if os.path.exists(images_npy_filename) and os.path.exists(image_sizes_npy_filename): #this check for existing numpy array, command out it and it will load the image everytime.
                                                                                          #problem: when finish task, we need to empty the input and output folder to prepare for the next task 
        #images = np.load(images_npy_filename)                                            #we can maintain the input output folder by cloning into another directory in case judges asking about how we can store the data.
        #image_sizes = np.load(image_sizes_npy_filename)
